...
async function fetchSite(url) {
  ...
  try {
    ...
    if (blocked || body.length < 40) return { title: '', desc: '', body: '', blocked: true, _status: res.status, _rawTitle: title, _bodyLen: body.length };
    return { title, desc, body, blocked: false, _status: res.status, _bodyLen: body.length };
  } finally {
    clearTimeout(timeout);
  }
}
...
export default async function handler(req, res) {
  ...
  let fetchErr = null;
  try { site = await fetchSite(url); } catch (e) { fetchErr = e.message; }

  if (req.query && req.query.debug === 'SCRAPE') {
    return res.status(200).json({ _debug: true, url, status: site._status || null, blocked: !!site.blocked, bodyLen: site._bodyLen ?? (site.body || '').length, rawTitle: site._rawTitle || site.title || '', bodySample: (site.body || '').slice(0, 300), fetchErr });
  }
